...
class IncrementalLearner(abc.ABC):
    ...
    the main modification is save_parameters's torch pth save to mindspore's ckpt save
    """
    ...
    """
    set_task_info:
    set the num of class, train data, test data for a task
    """
    ...
    def eval_task(self, data_loader):
        ...
        self.eval()
        print(self._eval_task(data_loader))
    ...
    """
    The 4 function below has been convert to MindSpore Ver
    """

    def save_parameters(self, directory, run_id):
        path = os.path.join(directory, f"net_{run_id}_task_{self._task}.ckpt")
        logger.info(f"Saving model at {path}.")
        ms.save_checkpoint(self.network, path)
    ...
    def load_parameters(self, directory, run_id):
        ...
        # try:
        load_param_into_net(self.network, load_checkpoint(path))

    # except Exception:
    #     logger.warning("Old method to save weights, it's deprecated!")

    def eval(self):
        self._network.set_train(mode=False)

    def train(self):
        self._network.set_train(mode=True)
